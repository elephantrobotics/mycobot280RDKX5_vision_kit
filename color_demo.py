    ...
    def get_frame(self):
        ...
        while True:
            ...
            for color_info in self.colors:
                total_mask = np.zeros_like(hsv[:, :, 0])
                for lower, upper in color_info["ranges"]:
                    mask = cv2.inRange(hsv, lower, upper)
                    total_mask = cv2.bitwise_or(total_mask, mask)
                processed_mask = cv2.morphologyEx(total_mask, cv2.MORPH_OPEN, self.kernel)
            ...
            if max_contour is not None:
                ...
                cv2.circle(frame,  tuple(map(int, rect[0])), 5, (0, 0, 255), -1)
                cv2.putText(frame, color_name, (int(rect[0][0]), int(rect[0][1]-10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 0), 2)
                cv2.imshow("result",frame)
                cv2.waitKey(1500)
                cv2.destroyAllWindows()
                return [cam_X ,cam_Y],yaw,color_name
            else:
                cv2.imshow("result",frame)
